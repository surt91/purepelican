function lunr_search(term) {
  if (!tipuesearch) {
    console.error("Tipue search plugin is required");
    return;
  }
  ...
  if (term && idx && documents) {
    ...
    const resultIntro = `
    <h1>${result_title_str} <span class="lunr-search-term">${term}</span></h1>
    `;
    ...
    if (results.length > 0) {
      ...
      const resultFailure = `<p class="lunr-result-fail">${fail_str} <span class="lunr-search-term">${term}</span></p>`;
